...
### Fixed
- **Uses far less mobile data.** The Caltrans lane-closure feeds are the largest thing the plugin downloads (the Los Angeles district feed alone is about 15 MB, and Caltrans does not compress it), and the plugin was re-downloading every district near you every 5 minutes while you drove, even when nothing had changed. A driver in Los Angeles could use roughly 280 MB per hour. It now refreshes closures every 15 minutes instead of every 5, which cuts that by about two thirds. Established closures last hours, so nothing useful is lost. The plugin also asks Caltrans "has this changed since last time?" before downloading, so an unchanged feed costs nothing. In practice Caltrans rewrites these files on a timer, so that check pays off less often than it could, but it never costs anything and it will help whenever a feed goes quiet. Chain controls ask the same question.
